...
```{r, echo = TRUE}
setwd("/home/nelson/projectRepRes1")
...
###Creating a histogram for the total number of steps taken each day
```{r, echo = TRUE}
library(ggplot2)
...
##Computing for the mean and median number of steps taken each day
...
###Creating a time series plot of the average number of steps taken
```{r, echo = TRUE}
Q4<-Q3
...
###Getting the 5-minute interval that, on average, contains the maximum number of steps
...
The following is the code to describe and to show a strategy for imputing missing data.
...
For the purpose of simplicity, in this question, I will be using the mean/mode value substitution strategy to impute missing values. That is, using the mean values to substitute out the missing values in the original data set
...
The  histogram of the total number of steps taken each day after missing values are imputed

```{r, echo = TRUE}
png("figure/plot7.png")
...
png("figure/plott8.1.png")
xyplot(data=Q8.1,Steps~interval|WDWE, grid = TRUE, type = c("p", "smooth"), lwd = 4,panel = panel.smoothScatter)
dev.off()
png("figure/plot8.2.png")
hexbinplot(data=Q8.1,Steps~interval|WDWE, aspect = 1, bins=50)
dev.off()
```
